	...
	socket.on("Room", function (data) {
		...
		if (alreadyPlay) {
			...
			if (roomSocket == undefined) {
				...
				socket.adapter.rooms[data.room].dealerValue = 0;
				socket.adapter.rooms[data.room].play = "0";
				socket.adapter.rooms[data.room].searchPlayers = 2;
				socket.adapter.rooms[data.room].waitingCount = 3;
				socket.adapter.rooms[data.room].gameTimer = 0;
				socket.adapter.rooms[data.room].curPlyValue = 0;
				socket.adapter.rooms[data.room].bootValue = parseInt(data.BootAmt);
				socket.adapter.rooms[data.room].potValue = 0;
				socket.adapter.rooms[data.room].chaalValue = parseInt(data.BootAmt);
				socket.adapter.rooms[data.room].blindValue = parseInt(data.BootAmt);
				socket.adapter.rooms[data.room].winPlayer = 0;
				socket.adapter.rooms[data.room].winStr = "";
				socket.adapter.rooms[data.room].shuffle = "";
				socket.adapter.rooms[data.room].roundCount = 0;
				socket.adapter.rooms[data.room].maxBlind = parseInt(data.MaxBlind);
				socket.adapter.rooms[data.room].potLimit = parseInt(data.PotLimit);
				socket.adapter.rooms[data.room].chaalLimit = parseInt(data.ChaalLimit);
				socket.adapter.rooms[data.room].limitType = data.limitType;
...
function RegisterMySql(data, lSocket) {
	...
	var post = {
		...
		verify_mobile: "not_verify"
	};
	pool.query('INSERT INTO players SET ?', post, function (error, result, fields) {
		if (error) {
...
function VerifyUser(data, lSocket, optValue) {
	...
	var sql = 'SELECT * FROM players WHERE email = ?';
		...
		for (var i in result) {
			...
			if (result[i].email == email && optValue == 0 && ppp) {
				...
				lSocket.emit("VerifyUser", {
					...
					mobile: result[i].mobile, verify_mobile: result[i].verify_mobile, password: data.password
				});
				succ = "success";
			}
		}
...
function Updated_Chips(lSocket, email, chips) {
	...
	var sql = "UPDATE players set chips = ? WHERE email = ?";
...
function Updated_Cash2(lSocket, email, cash) {
	var sql = "UPDATE players set cash = ? WHERE email = ?";
...
function ReferCommission(data, lSocket) {
	var sql = 'SELECT * FROM players WHERE referby_friends = ?';
...
function ReferCommission2(cashPlusCommission, data, lSocket) {
	var sql = "UPDATE players set cash = ? WHERE referral_code = ?";
...
function WithdrawSetDate(lSocket, data) {
	var sql = "UPDATE players set withdraw_limit = ? WHERE email = ?";
...
function ChangeMobileNumber(data, lSocket) {
	var sql = "UPDATE players set mobile = ? WHERE email = ?";
	pool.query(sql, [data.mobile, data.email], function (err, result) {
		if (err) {
			lSocket.emit("ChangeMobile", { status: "fail" });
		} else {
			lSocket.emit("ChangeMobile", { status: "success", phone: data.mobile });
		}
